...
def train_xgb_pipeline() -> Pipeline:
    ...
    log.debug(f"Training set: {len(genuine_texts)} genuine + {len(misleading_texts)} misleading = {len(X)} total")
    ...
    pipeline.fit(X, y)
    log.info(f"XGBoost pipeline trained successfully on {len(X)} examples")
    return pipeline


def xgb_pipeline_predict(text: str, pipeline: Pipeline) -> Dict[str, object]:
    """Run the baseline XGBoost pipeline and return its label and winning probability."""
